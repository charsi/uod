...
// google api key
var key = "AIzaSyDnfJIBZj1_q75mLz20h-tSft1gl5SeXFs";
...
var DriveInfo = function () {
	function DriveInfo(_ref) {
		var _ref$duration = _ref.duration,
		    duration = _ref$duration === undefined ? 0.00 : _ref$duration,
		    _ref$distance = _ref.distance,
		    distance = _ref$distance === undefined ? 0.00 : _ref$distance,
		    _ref$petrolMileage = _ref.petrolMileage,
		    petrolMileage = _ref$petrolMileage === undefined ? 14.00 : _ref$petrolMileage,
		    _ref$dieselMileage = _ref.dieselMileage,
		    dieselMileage = _ref$dieselMileage === undefined ? 14.00 : _ref$dieselMileage,
		    _ref$petrolCost = _ref.petrolCost,
		    petrolCost = _ref$petrolCost === undefined ? 0.00 : _ref$petrolCost,
		    _ref$dieselCost = _ref.dieselCost,
		    dieselCost = _ref$dieselCost === undefined ? 0.00 : _ref$dieselCost,
		    _ref$trafficMultiplie = _ref.trafficMultiplier,
		    trafficMultiplier = _ref$trafficMultiplie === undefined ? 1.00 : _ref$trafficMultiplie,
		    _ref$fuel = _ref.fuel,
		    fuel = _ref$fuel === undefined ? 'petrol' : _ref$fuel;

		_classCallCheck(this, DriveInfo);
	...
	_createClass(DriveInfo, [{
		key: 'getFuelUsed',
		value: function getFuelUsed() {
		...
		key: 'setFuelCost',
		value: function setFuelCost(cost) {
			this[this.fuel + 'Cost'] = cost;
		}
	}, {
		key: 'display',
		value: function display() {
			var dispUnits = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : units;
...
function resetEverything() {
	...
	$resultGrid.fadeOut("slow", function () {
		...
		[$uberResultSubDiv, $driveResultSubDiv, $resultDiv, $fromDiv, $toDiv, $fuelSpan].forEach(clearDiv);
	});
}
...
function fromLocationValid() {
	if (currentLocationInfo.start_latitude && currentLocationInfo.start_longitude && $sj_from.val()) {
		return true;
	} else {
		return false;
	}
...
function toLocationValid() {
	if (currentLocationInfo.end_latitude && currentLocationInfo.end_longitude && $('#locationToInput').val()) {
		return true;
	} else {
		return false;
	}
...
function refreshDriveInfo() {
	...
	$resultDiv.html('<p>Distance: ' + dv.distance + '</p>' + '<p><i class="material-icons" id="time-icon">access_time</i> ' + di.duration + ' mins</p>');
}
...
function initAutocomplete() {
	...
	/** @type {!HTMLInputElement} */document.getElementById('co_city'), _defineProperty({ types: ['geocode'] }, 'types', ['(cities)']));
	// populate 'from' when user selects an address from the list 
...
function commaSeparateNumber(val) {
	...
	} else while (/(\d+)(\d{3})/.test(val.toString())) {
...
function coCalculate() {
	...
	}, _defineProperty(_returnData, 'carFuelCost', carFuelCost), _defineProperty(_returnData, 'carInsurance', carInsurance), _defineProperty(_returnData, 'uberCostPerKm', ccf.uberCostPerKm), _returnData);
	return returnData;
}
